...
export async function rankTracker(keyword, url) {
    let browser;
    try {
        ...
        await page.goto(`https://www.google.com/search?q=${encodeURIComponent(keyword)}`, { waitUntil: 'networkidle' });
        let response;
        try {
        ...
        const scrapedData = await page.evaluate(() => {
            ...
            return {
                metaData: {
                    ...
                    headings,
                    links: {
                        internal: internalLinks, external: externalLinks, total: allLinks.length
                    },
        ...
        if (browser) {
            try {
                ...
                console.error("[SCRAPER] Failed to close browser after error:", error.message);
            }
        }
    }
}
...
export async function scrapeUrl(urlString) {
    ...
    try {
        ...
        let response;
        try {
            response = await page.goto(urlString, { waitUntil: 'domcontentloaded' })
        } catch (navError) {
            await browser.close().catch(() => { });
            browser = null;
            return { success: false, error: navError.message }
        ...
        return {
            success: true,
            data: { ...scrapedData, loadTime, statusCode, url: urlString }
        }
    } catch (error) {
        console.error("[SCRAPER] Playwright session failed:", error.message);
        if (browser) {
